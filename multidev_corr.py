...
class ProcessCorr():
    ...
    def _init_corr(self, min_val=None, max_val=None):
        ...
        self.cross_corr = cp.zeros((self.num_bins, self.fshape[0]*2-1, self.fshape[1]*2-1))
    ...
    def proc_glob(self, min_val=None, max_val=None, **kwargs):
        ...
        cross_corr_arr = mp.Array(ctypes.c_double, num_jobs*self.num_bins*int(np.product(2*np.array(self.fshape)-1)))
        ...
        self.cross_corr = np.frombuffer(cross_corr_arr.get_obj()).reshape((num_jobs, self.num_bins) + tuple(2*np.array(self.fshape)-1))
    ...
    def _mp_worker(self, rank, flist, corr_arr, cross_corr_arr, integ_arr, corrsq_arr, isums_arr, bin_hist, adu_thresh, norm):
        ...
        mycrosscorr = np.frombuffer(cross_corr_arr.get_obj()).reshape((num_jobs, self.num_bins) + tuple(2*np.array(self.fshape)-1))
    ...
    def _proc_file(self, fname, fnum, isums, **kwargs):
        ...
        while True:
            ...
            sfr_corr_tmp = cusignal.fftconvolve(sfr_tmp[bin_idx], fr_integ[::-1, ::-1])
            sfr_tmp[bin_idx] = cp.copy(fr_integ)
    ...
    def _proc_frame(self, frame_cpu, adu_thresh=120, norm=True):
        ...
        return cusignal.fftconvolve(sfr, sfr[::-1,::-1]), sfr

    def norm_corr(self):
        if self.num_bins > 1:
            integ = self.integ*self.np_mask
            integ_corr = signal.fftconvolve(integ, integ[:,::-1,::-1], axes=(1,2)) 
            ...
            ncorr = (self.corr[0] - 1/2 * (self.cross_corr[0] +
                                           self.cross_corr[0,::-1,::-1])) / signal.fftconvolve(self.integ[0], self.integ[0,::-1,::-1]) 
