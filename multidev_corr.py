...
class ProcessCorr():
    ...
    def proc_glob(self, min_val=None, max_val=None, **kwargs):
        ...
        cross_corr_arr = mp.Array(ctypes.c_double, num_jobs*self.num_bins*int(np.product(np.array(self.fshape))))
        ...
        self.cross_corr = np.frombuffer(cross_corr_arr.get_obj()).reshape((num_jobs, self.num_bins) + tuple(np.array(self.fshape)))
    ...
    def _mp_worker(self, rank, flist, corr_arr, cross_corr_arr, integ_arr, corrsq_arr, isums_arr, bin_hist, adu_thresh, norm):
        ...
        mycrosscorr = np.frombuffer(cross_corr_arr.get_obj()).reshape((num_jobs, self.num_bins) + tuple(np.array(self.fshape)))
