...
class ProcessCorr():
    ...
    def _init_corr(self, min_val=None, max_val=None):
        ...
        self.corr = cp.zeros((self.num_bins, 2*self.fshape[0], 2*self.fshape[1]))
    ...
    def proc_glob(self, min_val=None, max_val=None, **kwargs):
        ...
        corr_arr = mp.Array(ctypes.c_double, num_jobs*self.num_bins*int(np.product(2*np.array(self.fshape))))
        integ_arr = mp.Array(ctypes.c_double, num_jobs*self.num_bins*int(np.product(self.fshape)))
        corrsq_arr = mp.Array(ctypes.c_double, num_jobs*self.num_bins*int(np.product(2*np.array(self.fshape))))
        ...
        self.corr = np.frombuffer(corr_arr.get_obj()).reshape((num_jobs, self.num_bins) + tuple(2*np.array(self.fshape)))
    ...
    def _mp_worker(self, rank, flist, corr_arr, integ_arr, corrsq_arr, isums_arr, bin_hist, adu_thresh, norm):
        ...
        mycorr = np.frombuffer(corr_arr.get_obj()).reshape((num_jobs, self.num_bins) + tuple(2*np.array(self.fshape)))
    ...
    def _proc_file(self, fname, fnum, isums, **kwargs):
        ...
        while True:
            ...
            med = cp.median(fr_integ).tolist()
            fr_integ_pad = cp.pad(fr_integ, ((fr_integ.shape[0]//2, fr_integ.shape[0]//2), (fr_integ.shape[1]//2, fr_integ.shape[1]//2)), mode='constant', constant_values=((med,med), (med, med)))
            sfr_corr_tmp = cusignal.fftconvolve(sfr_tmp[bin_idx], fr_integ_pad[::-1, ::-1], mode='same')
    ...
    def _proc_frame(self, frame_cpu, adu_thresh=120, norm=True):
        ...
        med = cp.median(sfr).tolist()
        sfr_pad = cp.pad(sfr, ((sfr.shape[0]//2, sfr.shape[0]//2), (sfr.shape[1]//2, sfr.shape[1]//2)), mode='constant', constant_values=((med,med),(med,med)))
        return cusignal.fftconvolve(sfr_pad, sfr_pad[::-1,::-1], mode='same'), sfr
    ...
    def norm_corr(self):
        if self.num_bins > 1:
            ...
            integ = self.integ[0]
            med = np.median(integ)
            integ_pad = np.pad(integ, ((integ.shape[0]//2, integ.shape[0]//2), (integ.shape[1]//2, integ.shape[1]//2)), mode='constant', constant_values=((med,med),(med,med)))
            ncorr = self.corr[0] / signal.fftconvolve(integ_pad, integ_pad[::-1,::-1], mode='same') 
        return ncorr
